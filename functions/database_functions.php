<?php
	function db_connect(){
		$conn = mysqli_connect("localhost", "root", "root", "book_store_db", 8889);
		if(!$conn){
	...
	function getOrders($conn){
		$query = "SELECT * FROM orders left join customers on orders.customerid = customers.customerid";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getOrderItems($conn, $orderId){
		...
		$query = "SELECT * FROM order_items left join books on order_items.book_isbn = books.book_isbn where orderid = $orderId ";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
